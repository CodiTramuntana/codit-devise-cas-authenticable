  ...
  module SingleSignOut
    ...
    module DestroySession
      def destroy_session_by_id(sid)
        ...
        if session_store_class.name.match?(/ActiveRecord::SessionStore/)
          session = session_store_class::Session.find_by_session_id(sid)
          session&.destroy
          true
        elsif session_store_class.name.match?(/ActionDispatch::Session::ActiveRecordStore/)
          session = current_session_store.session_class.find_by_session_id(sid)
          session&.destroy
          true
        elsif session_store_class.name.match?(/ActionDispatch::Session::DalliStore/)
          current_session_store.send(:destroy_session, env, sid, drop: true)
          true
        elsif session_store_class.name.match?(/RedisSessionStore/)
          current_session_store.send(:destroy_session, env, sid, drop: true)
          true
        elsif session_store_class.name.match?(/Redis/)
          current_session_store.instance_variable_get(:@pool).del(sid)
          true
        elsif session_store_class.name.match?(/CacheStore/)
